...
void main() {
  ...
  test(
    ...
    () async {
      ...
      expect(prepared.manifest.schemaVersion, 24);
      expect(prepared.preparedSchemaVersion, 24);
      ...
      try {
        expect(database.userVersion, 24);
        expect(
  ...
  test('migra una copia v22 a 24 conservando datos e importes', () async {
    final databaseFile = await _createCurrentDatabase(
      temporaryDirectory,
      'legacy.sqlite',
      withInvoice: true,
    ...
    try {
      ...
      expect(migratedDatabase.userVersion, 24);
  ...
  test('rechaza una copia con una tabla obligatoria ausente', () async {
    ...
    await expectLater(
      service.prepare(backupPath: backup.path, currentSchemaVersion: 24),
