...
void main() {
  ...
  test(
    ...
    () async {
      ...
      expect(prepared.manifest.schemaVersion, 25);
      expect(prepared.preparedSchemaVersion, 25);
      ...
      try {
        expect(database.userVersion, 25);
        expect(
  ...
  test('migra una copia v22 a 25 conservando datos e importes', () async {
    final databaseFile = await _createCurrentDatabase(
      temporaryDirectory,
      'legacy.sqlite',
      withInvoice: true,
    ...
    try {
      ...
      expect(migratedDatabase.userVersion, 25);
  ...
  test('rechaza una copia con una tabla obligatoria ausente', () async {
    ...
    await expectLater(
      service.prepare(backupPath: backup.path, currentSchemaVersion: 25),
