...
void main() {
  late AppDatabase db;
  late CircuitoProveedorRepository repository;
  setUp(() async {
    db = AppDatabase.forTesting(NativeDatabase.memory());
    await db.ensureReady();
    await db.expedientesDao.insertarExpediente(
      ExpedientesCompanion.insert(id: 'obra-a', codigo: 'A', nombre: 'Obra A'),
    );
    await db.expedientesDao.insertarExpediente(
      ExpedientesCompanion.insert(id: 'obra-b', codigo: 'B', nombre: 'Obra B'),
    );
    await db.proveedoresDao.insertarProveedor(
      ProveedoresCompanion.insert(id: 'prov', nombre: 'BigMat'),
    );
  ...
  test('crea un albarán con líneas y reparto multiobra sin coste', () async {
    await repository.crearAlbaran(
      AlbaranInput(
        ...
        lineas: const [
          LineaAlbaranInput(
            descripcion: ' Bloque H ',
            cantidad: 10,
            importeCentimos: 50000,
            asignaciones: [
              ...
              AsignacionImporteInput(importeCentimos: 5000),
            ],
          ),
        ],
      ),
    ...
    expect(
      (await db.select(db.lineasAlbaranProveedor).get())
          .single
          .descripcionOriginal,
      'Bloque H',
    );
  ...
  test(
    'factura multiobra agrupa albaranes y detecta duplicado normalizado',
    () async {
      final albaran = await repository.crearAlbaran(
        AlbaranInput(
          proveedorId: 'prov',
          referencia: 'A-2',
          fecha: DateTime(2026, 9, 5),
          lineas: const [LineaAlbaranInput(descripcion: 'Arena', cantidad: 1)],
        ),
      );
      await repository.crearFactura(
        FacturaRecibidaInput(
          ...
          asignaciones: const [
            AsignacionImporteInput(
              expedienteId: 'obra-a',
              importeCentimos: 60000,
            ),
            AsignacionImporteInput(
              expedienteId: 'obra-b',
              importeCentimos: 30000,
            ),
            AsignacionImporteInput(importeCentimos: 10000),
          ],
        ),
      );
      expect(await db.select(db.facturaRecibidaAlbaranes).get(), hasLength(1));
      expect(
        await db.select(db.asignacionesFacturaRecibida).get(),
        hasLength(3),
      );
      await expectLater(
        repository.crearFactura(
          FacturaRecibidaInput(
            proveedorId: 'prov',
            numero: 'f00126',
            fecha: DateTime(2026, 9, 5),
            baseCentimos: 1,
            ivaCentimos: 0,
            asignaciones: const [AsignacionImporteInput(importeCentimos: 1)],
          ),
        ),
        throwsA(anything),
      );
  ...
  test(
    'reconoce una vez, excluye IVA recuperable y pago no crea coste',
    () async {
      final facturaId = await repository.crearFactura(
        FacturaRecibidaInput(
          proveedorId: 'prov',
          numero: 'F-2',
          fecha: DateTime(2026, 9, 5),
      ...
      await repository.consolidarFactura(facturaId);
      await repository.verificarPago(facturaId, motivo: 'Prueba ficticia');
      final asignacion =
          (await db.select(db.asignacionesFacturaRecibida).get()).single;
      await repository.reconocerAsignacion(asignacionId: asignacion.id);
      await repository.reconocerAsignacion(asignacionId: asignacion.id);
      final hechos = await db.select(db.hechosCoste).get();
      expect(hechos, hasLength(1));
      expect(hechos.single.importeCosteCentimos, 50000);
      await repository.registrarPago(
        facturaId: facturaId,
      ...
      await repository.registrarPago(
        ...
        importeCentimos: 50500,
      );
      expect(
        (await db.select(db.facturasRecibidas).get()).single.estado,
        'pagada',
      );
      await expectLater(
