...
**PROD-6 — preparado para aceptación manual en Desarrollo (2026-09-15):** auditoría prepiloto de 28 áreas realizada antes de modificar código. Corregido el destinatario empresarial de nuevas facturas para coincidir con el presupuesto; originales históricos conservados. Restauración con aviso explícito de retroceso fiscal y reconocimiento obligatorio. Sin pantalla nueva de diagnóstico: se reutilizan entorno, configuración, series, integridad documental y backup validado. Schema 36, sin migración. Verificación: 7 pruebas nuevas, 456 totales, analyze sin incidencias, formato de 23 Dart, Windows debug aislado con 368 archivos idénticos y diff sin errores. GO técnico para aceptación manual; NO-GO para uso real hasta aceptación y autorización expresa, revisión de gestoría y copia externa verificable. Sin push, datos operativos reales, configuración de Producción ni Fase 5. [Auditoría, manual DEV y checklist real separado](PROD6_PILOTO_REAL.md). [Informe técnico](PROD6_INFORME.md).

**PROD-6 — corrección de aceptación (2026-09-16):** la asignación inicial de una factura recibida en borrador queda distinguida de una reimputación posterior a la consolidación. La regresión borrador sin obra → obra → consolidación demuestra un único coste de 400,00 €. Schema 36, sin migración; 23 pruebas del circuito y 457 totales superadas, analyze limpio. Nuevo Windows debug aislado compilado en `build/prod6-blocker-verification-20260916`, sin ejecutar ni abrir bases, y con 366 entradas verificadas byte a byte. Se mantiene NO-GO hasta repetir el paso en la aceptación manual de Desarrollo y completar el resto de condiciones del piloto; sin Producción, datos reales ni push.

**PROD-6 — logotipo documental corregido (2026-09-16):** los PDF FAC/RECT respetan el logotipo configurado, conservan el predeterminado solo sin ruta y bloquean una ruta ilegible o una imagen inválida antes de completar la emisión. La transacción conserva borrador, numeración y ausencia de PDF ante error. Verificación: 35 pruebas relacionadas, 461 totales, analyze limpio y Windows debug aislado en `build/prod6-logo-verification-20260916`. P6-01 continúa en borrador; se mantiene NO-GO hasta reanudar y completar la aceptación manual. Schema 36; sin Producción, datos reales ni push.

**PROD-6 — GO manual en Desarrollo (2026-09-16):** aceptación completada con factura recibida, coste/pago, factura emitida/cobro, PDF histórico con logotipo configurado, bloqueos de duplicados y saldos, integridad documental, restore con numeración y persistencia tras reapertura. Backup final `obraia-1789585288035.obraia-backup` conservado también en OneDrive; aplicación DEV cerrada. La revisión previa al commit deja la composición de Dashboard y Búsqueda en `presentation/providers`, sin dependencias `data → presentation` ni cambios funcionales. PROD-6 queda aceptado solo en Desarrollo. Producción y datos reales siguen fuera; el GO no autoriza despliegue, commit ni push, y cualquier entrada real requiere autorización independiente y los controles de gestoría definidos.

Las entradas siguientes son históricas: las menciones a PROD-6 no iniciado o a publicación pendiente de PROD-5 describen aquellos cierres. El encargo actual confirma PROD-1 a PROD-5 publicados y cerrados; no se reabren.

**PROD-5 — cierre técnico final y corrección validada manualmente (2026-09-10):** revisión del diff completo sin bloqueos para publicar dentro del alcance documentado. El usuario confirma en Desarrollo: vacío histórico identificado, rechazo de 0 bytes, alta válida de 62 bytes, integridad y exportación correctas, y conservación independiente de A/B al reutilizar la misma ruta. Verificaciones repetidas: formato de 46 Dart sin cambios, analyze sin incidencias, 71 pruebas específicas y 449 totales superadas; Windows debug aislado compilado con 362 archivos relevantes idénticos. Schema 36. Solo documentación modificada en este cierre; analysis_options.yaml permanece intacto y fuera. GO técnico para publicación, pendiente de autorización de push. No se han modificado datos ni usado Producción; PROD-6 y Fase 5 no iniciados. [Evidencia y límites](PROD5_DOCUMENTOS_BACKUP.md).

**Histórico: entrega inicial de PROD-5, anterior al cierre del 2026-09-10:** originales gestionados por entorno y tenant, integridad SHA-256, backup v2 con inventario de SQLite y documentos, staging y rollback documental. Schema 36 con migración aditiva desde 35. Históricos externos identificados sin copia automática; PDFs congelados conservados en SQLite. Verificación final: 439 pruebas superadas (26 nuevas de PROD-5: 9 de originales/repositorio, 13 de backup documental, 2 de UI, 1 de migración y 1 adicional de rollback); analyze sin incidencias; Windows debug compilado en copia aislada con 361 archivos relevantes idénticos; formato propio y git diff --check sin errores. Pendiente aceptación manual. Sin push, datos reales, cambios en Producción, PROD-6 ni Fase 5. [Alcance, límites, restauración y aceptación](PROD5_DOCUMENTOS_BACKUP.md).
...
**PROD-2 — técnicamente preparado y verificado:** presupuesto aceptado con snapshot comercial, PDF original y SHA-256 dentro de SQLite; aceptación atómica con plan económico y Timeline, bloqueo de edición, referencias protegidas por tenant y tratamiento explícito de legacy. Schema 33. Análisis sin incidencias, 33 pruebas nuevas (29 documentales, 3 UI y 1 migración), suite completa de 366 pruebas y Windows debug en copia aislada superados; diff sin errores. PROD-1 publicado y validado manualmente según confirmación del usuario. PROD-2 publicado, validado manualmente en DEV y cerrado; Fase 5 no iniciada. Detalle y procedimiento en [PROD2_PRESUPUESTO_SEGURO.md](PROD2_PRESUPUESTO_SEGURO.md).
...
**Siguiente hito: PUERTA DE ENTRADA A PRODUCCIÓN / PILOTO REAL**, antes de continuar con Fase 5. Debe incluir revisión operativa y protección de documentos originales: PROD-5 incorpora los originales gestionados al backup v2; las referencias externas siguen fuera. Quedan aceptación manual, copias verificadas fuera del equipo y recuperación asistida en una instalación nueva antes de considerarlo archivo documental único.
