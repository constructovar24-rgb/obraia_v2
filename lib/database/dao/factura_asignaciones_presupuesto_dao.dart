...
class FacturaAsignacionesPresupuestoDao extends DatabaseAccessor<AppDatabase>
  ...
  Future<void> insertar(FacturaAsignacionesPresupuestoCompanion value) =>
      into(facturaAsignacionesPresupuesto).insert(value);
  ...
  Stream<List<domain.FacturaAsignacionPresupuesto>> observarTodas() => select(
    facturaAsignacionesPresupuesto,
  ).watch().map((rows) => rows.map(_toDomain).toList());

  Future<void> eliminarPorFactura(String facturaId) => (delete(
    facturaAsignacionesPresupuesto,
  ...
  Future<bool> existePorLineaPresupuesto(String lineaId) async =>
      await (select(facturaAsignacionesPresupuesto)
            ..where((t) => t.lineaPresupuestoId.equals(lineaId))
            ..limit(1))
          .getSingleOrNull() !=
      null;
}
